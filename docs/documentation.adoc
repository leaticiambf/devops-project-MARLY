...
The browser targets the frontend origin only. User-facing routes such as `/`, `/tasks`, `/login`, and `/eco-score` are owned by the Next.js application, while the backend exposes API, OAuth callback, health, and logout endpoints.
