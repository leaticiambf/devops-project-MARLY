...
together {
  rectangle "**Frontend** (Next.js App Router)" as Frontend #lightyellow
}
...
|`POST /api/users` (register), `POST /api/users/login`, `/api/auth/login`, `/oauth2/**`, `/login/oauth2/**`, `/actuator/health`, `/actuator/info`, `/favicon.ico`
...
3. **Create OAuth2 Credentials**:
   ...
   - Authorized redirect URIs:
     * `http://localhost:3000/login/oauth2/code/google` (development)
...
* Route entries for journey planning, tasks, eco score, comfort presets, authentication, and Google popup completion states
...
* **comfort-presets/** — Mobile-friendly management of named comfort profiles used by the planner
...
* **map/** — Standalone transport map surface without demo itinerary rendering when no journey is active
...
* Reusable cards, buttons, state panels, mobile-safe toasts, and layout primitives

=== Route Ownership

The browser targets the frontend origin only. User-facing routes such as `/`, `/tasks`, `/comfort-presets`, `/login`, and `/eco-score` are owned by the Next.js application, while the backend exposes API, OAuth callback, health, and logout endpoints.
...
4. Configure redirect URIs:
   - Development: `http://localhost:3000/login/oauth2/code/google`
...
* **Frontend Migration**: The UI was moved into the standalone `maview/` Next.js application with route-based pages, shared state providers, and frontend rewrites to the Spring backend
* **Test Coverage Improvement**: Increased from ~60% to ~85%+ with comprehensive unit and integration tests
* **Frontend Cutover**: Legacy backend-served UI routes now redirect to the external frontend origin instead of serving a bundled HTML shell
...
* **Controllers**: 6 (JourneyController, UserController, GoogleTasksController, DisruptionController, EcoScoreController, AuthController)
